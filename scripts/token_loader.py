...
class TokenManager:
    ...
    def extract_unrecognized_tokens_from_results(
    ...
    def load_tokens_from_file(self, results_file: str = None) -> Set[str]:
        ...
        try:
            ...
            for success in data.get("successful_extractions", []):
                tokens = self.extract_unrecognized_tokens_from_results(
                    success.get("data", {})
                )
    ...
    def get_stablecoin_decision(self, token: str) -> bool:
        ...
        while True:
            ...
            if response in ["y", "yes"]:
                ...
                print(
                    "Please enter 'y' for yes, 'n' for no, or 'skip' to skip this token"
                )

    async def add_token(
        self, session: aiohttp.ClientSession, token: str, is_stable: bool
    ) -> Dict:
        ...
        async with aiohttp.ClientSession() as session:
            for token, is_stable in tokens_with_stability:
                ...
                if result["status"] == "success":
                    ...
                    print(
                        f"❌ Failed to add {token}: {result.get('error', 'Unknown error')}"
                    )
...
async def main():
    ...
    if not unrecognized_tokens:
        print(
            "No unrecognized tokens found. Make sure you have run the extraction script first."
        )
        return
    ...
    output_file = os.getenv(
        "TOKEN_ADDITION_RESULTS_FILE", "token_addition_results.json"
    )
