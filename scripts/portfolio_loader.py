...
class PortfolioLoader:
    ...
    async def extract_transaction(self, session: aiohttp.ClientSession, 
                    ...
                    if response.status == 200:
                        ...
                        if result.get("status") == "success":
                            ...
                            return {
                                ...
                                "image_path": image_path,
                                "message": result.get("message", "No transactions found"),
                                "data": result
                            }
    ...
    async def process_screenshots(self, screenshot_paths: List[str], 
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    print(f"❌ Exception processing {os.path.basename(screenshot_paths[i])}: {result}")
                    continue
                ...
                if result["status"] == "success":
                    ...
                    print(f"✅ Successfully processed: {filename} ({tx_count} transactions found)")
    ...
    def get_image_files(self, directory: str, extensions: List[str] = None) -> List[str]:
    ...
    def save_results(self, output_file: str = "extraction_results.json") -> None:
        ...
        results = {
            ...
            "summary": {
                "total_images_processed": len(self.successful_transactions) + len(self.errors),
    ...
    def print_summary(self) -> None:
        ...
        success_rate = (len(self.successful_transactions) / total_images * 100) if total_images > 0 else 0
        ...
        print(f"\n{'='*60}")
        print(f"DEBANK TRANSACTION EXTRACTION SUMMARY")
        print(f"{'='*60}")
...
async def main():
    ...
    # Process screenshots
    await loader.process_screenshots(
        screenshot_paths, 
